...
function CalendarDayView:onNextPage()
	if not self:nextPage() and self.day_ts + 82800 < os.time() then
		local current_day_ts = self.day_ts
...
function CalendarDayView:refreshTimeline()
	...
	local idx_00h00
	if
		self.reader_statistics.settings.calendar_day_start_hour
		and self.reader_statistics.settings.calendar_day_start_hour ~= 0
	then
...
function CalendarView:showCalendarDayView(reader_statistics)
	...
	if
		...
		< (reader_statistics.settings.calendar_day_start_hour or 0) * 3600
			+ (reader_statistics.settings.calendar_day_start_minute or 0) * 60
	then
	...
	UIManager:show(CalendarDayView:new({
		day_ts = os.time({
			...
			month = date.month,
			day = date.day,
			hour = reader_statistics.settings.calendar_day_start_hour or 0,
			min = reader_statistics.settings.calendar_day_start_minute or 0,
		}),
